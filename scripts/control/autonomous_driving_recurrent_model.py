...
from data import ResizeWithLabels, RGBTo3CGrayscale, ToTensorWithLabel, ComposeTransformations, CarDataset
from model import Model, RecurrentModel, load_best_model
...
if __name__ == "__main__":
    ...
    # command[4] - no action
        ...
        # Function that registers the input from the keyboard.
    def register_input():
        global quit, restart
    ...
    # Function for converting label to an action.
    ...
    # Define the transformations for the model input.
    transform = ComposeTransformations([
        ResizeWithLabels(),
        # RGBTo3CGrayscale(),
        ToTensorWithLabel()
    ])
    rand_label = [0, 0, 0, 0, 0]

    # Load the model for autonomous driving.
        ...
        while True:
            if steps % 1 == 0 or truncated or terminated and steps >= 25:
                ...
                with torch.no_grad():
                    ...
                    _, predicted = torch.max(output[-1, :], dim=0) # Take the last time sample from the prediction.
                    label = predicted.item()

                # Extract the commands for driving the car.
                a = label_to_action(label)
                a = np.array(a)
                print(label)
                print(a)
